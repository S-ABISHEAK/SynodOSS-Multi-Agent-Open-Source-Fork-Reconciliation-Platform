...
# ──────────────────────────────────────────────────────────────────────────────
# Enterprise Policy Engine (EPACE)
# ──────────────────────────────────────────────────────────────────────────────
...
class PolicyCategory(str, enum.Enum):
    ...
    ENTERPRISE_CUSTOM = "Enterprise Custom Rules"
...
class EnterprisePolicy(Base):
    """
    Stores metadata for an uploaded enterprise policy document.
    The actual text is stored chunked in PolicyChunk rows.
    """
    __tablename__ = "enterprise_policies"
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String, nullable=False)            # Human-readable policy name
    description = Column(Text, nullable=True)        # Short description
    category = Column(Enum(PolicyCategory))          # Classified category
    priority = Column(String, default="MEDIUM")      # LOW | MEDIUM | HIGH | CRITICAL
    version = Column(String, default="1.0")          # e.g. "1.0", "2.1"
    ...
    file_hash = Column(String, nullable=True, index=True)  # SHA256 — prevents re-embedding unchanged files
    total_chunks = Column(Integer, default=0)        # Number of chunks generated
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class PolicyChunk(Base):
    """
    Stores individual text chunks of a policy document, each with its embedding vector.
    Retrieved at debate time via cosine similarity against the conflict context.
    """
    __tablename__ = "policy_chunks"
    id = Column(Integer, primary_key=True, index=True)
    policy_id = Column(Integer, ForeignKey("enterprise_policies.id"), index=True)
    chunk_index = Column(Integer, nullable=False)    # Position in document
    chunk_text = Column(Text, nullable=False)        # The raw chunk text (≤300 tokens)
    chunk_hash = Column(String, nullable=True)       # Hash to detect duplicates
    embedding = Column(JSON, nullable=True)          # Stored as list[float]
    token_count = Column(Integer, default=0)
    created_at = Column(DateTime, default=datetime.utcnow)


class PolicyImpactResult(Base):
    """
    Stores the post-debate policy impact analysis result for a specific debate.
    Generated by PolicyAnalyzer after Round 4 (Verification Judge).
    """
    __tablename__ = "policy_impact_results"
    id = Column(Integer, primary_key=True, index=True)
    debate_id = Column(Integer, ForeignKey("debates.id"), unique=True, index=True)
    affected_policies = Column(JSON, nullable=True)  # list of {policy_id, name, category, reason}
    risk_level = Column(Enum(RiskLevel), default=RiskLevel.LOW)
    business_impact = Column(Text, nullable=True)
    escalation_needed = Column(Boolean, default=False)
    policy_impact_score = Column(Float, default=0.0)  # 0.0-1.0, used in Trust Score
    created_at = Column(DateTime, default=datetime.utcnow)
